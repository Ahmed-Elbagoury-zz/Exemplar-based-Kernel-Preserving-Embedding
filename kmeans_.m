function cluster_labels = kmeans_(data, centers, num_clusters, NumRp, MaxIt, Thr)
...
% Modified by Ahmed Farahat to run mutliple time and select the solution
% with the minium objective function
% It has been noted that the random initialization with many replicates outperform
...
centers0 = centers;
nMaxItrsReached = 0;    % Number of times the maximum iterations reached
...
for nIdTr = 1:NumRp
    ...
    %
    if strcmp(centers0, 'random')
      %disp('Random initialization...');
      centers = random_init(data, num_clusters);
    elseif isempty(centers0)
    ...
    while 1
      iter = iter + 1;
      if iter>MaxIt
         vObjectiveFn(nIdTr) = qnew;
         cClassLabels{nIdTr} = ind';
         nMaxItrsReached = nMaxItrsReached + 1;         
         break;
      end
      ...
      % display(num2str([iter, abs((qnew-qold)/qold)]))
      if threshold >= abs((qnew-qold)/qold)
        mesg = sprintf('\nkmeans converged!');
        
        % Added by Ahmed Farahat
        vObjectiveFn(nIdTr) = qnew;
        cClassLabels{nIdTr} = ind';
        % disp(mesg);
        break;
...
if nMaxItrsReached > 0
   mesg = sprintf('Max number of iterations reached %d of %d times!', nMaxItrsReached, NumRp);        
   disp(mesg);
end

[~, nIdMinTr] = min(vObjectiveFn);
cluster_labels = cClassLabels{nIdMinTr};
% cluster_labels = ind';
